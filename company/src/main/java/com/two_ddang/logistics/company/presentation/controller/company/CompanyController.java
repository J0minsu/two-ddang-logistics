...
public class CompanyController {
    ...
    @PostMapping
    public ResponseEntity<ResponseDTO<CreateCompanyResponse>> createCompany(@RequestBody CreatedCompanyRequest createdCompanyRequestDto) {
        return ResponseEntity.ok(ResponseDTO.okWithData(companyService.createCompany(createdCompanyRequestDto)));
    }

    @GetMapping
    public ResponseEntity<ResponseDTO<Page<CompanyResponse>>> getCompanies(@PageableDefault(page = 0, size = 10, sort = "createdAt", direction = Sort.Direction.DESC) Pageable pageable,
    ...
    public ResponseEntity<ResponseDTO<UpdateCompanyInfoResponse>> updateCompanyInfo(@PathVariable UUID companyId,
                                                                                    @RequestBody UpdateCompanyInfoRequest updateCompanyRequestDto)
    {
        return ResponseEntity.ok(ResponseDTO.okWithData(
                companyService.updateCompanyInfo(companyId, updateCompanyRequestDto)
        ));
    }
